...
(function () {
    ...
    class Api extends EventEmitter {
        ...
        byteOrder() {
            return this.send(Command.byteOrder());
        }
        calibrate(refine) {
            return this.send(Command.calibrate(refine));
        }
        closeMeasurement() {
            return this.send(Command.closeMeasurement());
        }
        getCaptureC3d() {
            return this.send(Command.getCaptureC3d());
        }
        getCaptureQtm() {
            return this.send(Command.getCaptureQtm());
        }
        getCurrentFrame() {
            return this.send(Command.getCurrentFrame.apply(Command, arguments));
        }
        getParameters() {
            return this.send(Command.getParameters.apply(Command, arguments));
        }
        getState() {
            return this.send(Command.getState());
        }
        ...
        releaseControl() {
            return this.send(Command.releaseControl());
        }
        reprocess() {
            return this.send(Command.reprocess());
        }
        setParameters() {
        ...
        bootstrap() {
            ...
            this.client.on(
                "data",
                function (chunk) {
                    this.mangler.read(chunk, this.options.byteOrder, {
                        fun: this.receivePacket,
                        thisArg: this,
                    });
                }.bind(this)
            );
        ...
        connect(port, host, major, minor) {
            if (this.client !== null) return Q.reject();
        ...
        discover(port) {
            if (_.isUndefined(port)) port = this.options.discoverPort;
            ...
            return new Promise((resolve, reject) => {
                ...
                server.on("message", (msg, rinfo) => {
                    ...
                    var discoverPacket = Packet.create(
                        msg,
                        this.options.byteOrder,
                        rinfo.address,
                        rinfo.port
                ...
                client.on("listening", () => {
                    ...
                    if (this.options.debug)
                        this.logger.logPacket(
                            Packet.create(buf, this.options.byteOrder)
                        );
        ...
        frequency(freq) {
            if (isNaN(freq) && freq !== "AllFrames")
                throw new TypeError(
                    "Frequency must be a number or 'AllFrames'"
                );
        ...
        receivePacket(data) {
            ...
            if (this.options.debug) this.logger.logPacket(packet);

            if (packet.type === qtmrt.EVENT) {
                if (command && command.data === "GetState")
                    this.promiseQueue.pop().resolve({
                        id: packet.eventId,
                        name: packet.eventName,
                    });
                ...
                if (command) {
                    if (command.data && command.data.startsWith("ByteOrder")) {
                        this.promiseQueue
                            .pop()
                            .resolve(
                                /little endian/.test(packet.data)
                                    ? "little endian"
                                    : "big endian"
                            );
                    } else if (
                        command.data &&
                        command.data.startsWith("QTMVersion")
                    ) {
                        var human =
                                "QTM " +
                                packet.data
                                    .replace("QTM Version is ", "")
                                    .slice(0, -1),
                            version = human.match(
                                /QTM (\d+)\.(\d+)(?: (?:Beta)|(?:Alpha))? \(build (\d+)\)/
                            );
                        this.promiseQueue.pop().resolve({
                            major: version[1],
                            minor: version[2],
        ...
        save(filename, overwrite) {
            ...
            if (arguments.length > 1) overwrite = "overwrite";
        ...
        setQtmEvent(label) {
            if (arguments.length < 1) throw new TypeError("No label specified");
        ...
        stopStreaming() {
            if (!this._isStreaming) {
                this.logger.log(
                    "Cannot stop streaming, not currently streaming",
                    "grey"
                );
                this.emit("end");
            ...
            return this.send(Command.stopStreaming()).then(() => {
                this.emit("end");
            });
        }
